...
module.exports.load = async function (app, ifValidAPI, ejs) {
  ...
  for (const {
    ...
    date,
    action,
  } of await process.db.getAllRenewalTimers()) {
    let the_date = parseFloat(date);

    const time_left = the_date - now;
    ...
    if (time_left <= 0) {
      if (action === "suspend" || action === "auto") {
        ...
        if (action === "auto") {
          ...
          const server_info_req = await fetch(
          ...
          const server_info = await server_info_req.json();

          const cost = process.env.renewal.renew_fee;

          const user_info = await process.db.fetchAccountPterodactylID(
            server_info.attributes.user
          );
          ...
          if (user_info) {
            ...
            if (coins >= cost) {
              ...
              let another_date_check =
                the_date + process.env.renewal.renewal_time;
              ...
              if (another_date_check - now <= 0) {
                ...
                while (!end_loop) {
                  ...
                  if (!end_loop)
                    another_date_check =
                      another_date_check + process.env.renewal.renewal_time;
                }
        ...
        if (dosuspendactions) {
          ...
          const time_left_2 = the_date - now;
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      if (
        current.memory > total.memory ||
        current.disk > total.disk ||
        current.cpu > total.cpu ||
        current.servers > total.servers
      )
        return functions.doRedirect(
          req,
          res,
          redirects.cannotrenewduetohavingmoreresourcesthanyoushouldbeabletohave
        );

      const server_id = req.params.id;
      if (
        req.session.data.panelinfo.relationships.servers.data.filter(
          (server) => server.attributes.id.toString() === server_id
        ).length !== 1
      )
        return functions.doRedirect(req, res, redirects.invalidserver);

      const renewal_date = await db.getSingleRenewalDate(server_id);
      if (renewal_date.timer === "???")
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      if (renewal_date.action === "deletion")
        return functions.doRedirect(
          req,
          res,
          redirects.servermustberenewedfirstbecauseitisgoingtogetdeleted
        );
      if (parseFloat(renewal_date.timer) - Date.now() - 7000 <= 0)
        return functions.doRedirect(req, res, redirects.fewsecondstorenew);
...
module.exports.set = async function (server_id, timer, action) {
  ...
  renewal_timers[server_id] = setTimeout(async () => {
    const renewal_date = await db.getSingleRenewalDate(server_id);
  ...
  if (!timer)
    process.db.runDBTimerActions(
      server_id,
      Date.now() + process.env.renewal.renewal_time,
      action
    );
};
...
module.exports.setDeletion = async function (server_id, timer) {
  ...
  if (!timer)
    process.db.runDBTimerActions(
      server_id,
      Date.now() + process.env.renewal.deletion_time,
      "deletion"
    );
};
