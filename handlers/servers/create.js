...
const renew_server = require("./renew.js");
...
module.exports.load = async function (app, ifValidAPI, ejs) {
  app.post(
    ...
    async (req, res) => {
      ...
      if (current.servers >= total.servers)
        return functions.doRedirect(req, res, redirects.exceedsserverplan);

      const location = req.body.location;
      if (typeof location !== "string")
        return functions.doRedirect(req, res, redirects.locationmustbeastring);
      if (!process.env.locations[location])
        return functions.doRedirect(req, res, redirects.invalidlocation);

      if (!process.env.locations[location].enabled)
        return functions.doRedirect(req, res, redirects.locationdisabled);

      const egg = req.body.egg;
      if (typeof egg !== "string")
        return functions.doRedirect(req, res, redirects.eggmustbeastring);
      if (!process.env.eggs[egg])
        return functions.doRedirect(req, res, redirects.invalidegg);
      ...
      if (Array.isArray(process.env.locations[location].package)) {
        if (!process.env.locations[location].package.includes(packageinfo.id)) {
          return functions.doRedirect(
            req,
            res,
            redirects.lockedlocationfrompackage
          );
        }
      }
      ...
      if (typeof name !== "string")
        return functions.doRedirect(req, res, redirects.namemustbeastring);
      if (name.length < 1)
        return functions.doRedirect(req, res, redirects.nameistooshort);
      if (name.length > 191)
        return functions.doRedirect(req, res, redirects.nameistoolong);
      ...
      if (!memory || !disk || !cpu)
        return functions.doRedirect(req, res, redirects.invalidresources);
      ...
      if (process.env.locations[location].renewal)
        await renew_server.set(serverinfo.attributes.id);

      setTimeout(
        // setTimeout() to prevent the renewal timer accidently saying "None."
        () => {
