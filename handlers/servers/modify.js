...
const suspendCheck = require("./suspension_system.js");
...
module.exports.load = async function (app, ifValidAPI, ejs) {
  app.post(
    ...
    async (req, res) => {
      ...
      const eggs = Object.entries(process.env.eggs).filter(
        (egg) => server.attributes.egg === egg[1].info.egg
      );
      if (eggs.length !== 1)
        return functions.doRedirect(req, res, redirects.missingegg);
      const egg = eggs[0][1];

      const { total, current } = await functions.getUserResource(req);

      if (!req.body.memory && !req.body.disk && !req.body.cpu)
        return functions.doRedirect(
          req,
          res,
          redirects.missingresources,
          `?id=${req.params.id}`
        );
      if (
        !(await functions.parseNumber(req.body.memory)) ||
        !(await functions.parseNumber(req.body.disk)) ||
        !(await functions.parseNumber(req.body.cpu))
      )
        return functions.doRedirect(
          req,
          res,
          redirects.invalidresources,
          `?id=${req.params.id}`
        );
      ...
      if (
        memory + current.memory - server.attributes.limits.memory >
          total.memory ||
      ...
      if ((await serverinfo_req.statusText) !== "OK")
        return functions.doRedirect(
          req,
          res,
          redirects.erroronmodification,
          `?id=${req.params.id}`
        );

      const serverinfo = await serverinfo_req.json();

      const new_all_server_data =
        req.session.data.panelinfo.relationships.servers.data.filter(
          (server) => server.attributes.id.toString() !== server_id
        );
      new_all_server_data.push(serverinfo);
      req.session.data.panelinfo.relationships.servers.data =
        new_all_server_data;
