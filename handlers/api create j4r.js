...
module.exports.load = async function (app, ifValidAPI, ejs) {
  app.post("/api/j4r/create", async (req, res) => {
    ...
    if (req.session.data && req.session.data.panelinfo.root_admin) {
      ...
      const j4r_invite = req.body.invite;

      // Felt demotivated when making this, so lazy. - Two
      if (typeof j4r_invite !== "string")
        return res.json({ error: process.api_messages.j4r.j4ridisnotastring });
      if (typeof j4r_id !== "string")
        return res.json({ error: process.api_messages.j4r.j4ridisnotastring });
      if (j4r_id.length === 0)
        return res.json({ error: process.api_messages.j4r.j4ridcannotbeempty });
      if (typeof server_id !== "string")
        return res.json({
          error: process.api_messages.j4r.serveridisnotastring,
        });

      if (typeof days !== "number")
        return res.json({ error: process.api_messages.j4r.daysisnotanumber });
      days = Math.round(days);
      if (days < 1)
        return res.json({
          error: process.api_messages.j4r.dayscannotbelessthanone,
        });
      if (days > 365)
        return res.json({
          error: process.api_messages.j4r.dayscannotbegreaterthan365,
        });
      var date = new Date();
      const expires_on = date.addDays(Number(days));

      const check_exists = await process.db.checkIfJ4RWithNameExists(j4r_id);
      if (check_exists)
