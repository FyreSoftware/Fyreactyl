  ...
  app.post(
    ...
    async (req, res) => {
      ...
      if (req.body.packages && req.body.packages !== null) {
        ...
        } else if (package && package.price > current.coins) {
          req.session.variables = {
            amount_needed: package.price - current.coins,
          };
        ...
        for (let [type, amount] of Object.entries(resources)) {
          if (process.env.store[type].enabled) {
            ...
            if (typeof amount === "object" && amount == null) {
              ...
              if (amount < 0) {
                test_if_all_zero++;
              } else if (amount + current[type] > 1073741823) {
                ...
                hardcoded_errors.push(
                  `The provided value for ${type} is too big.`
                );
              } else {
                ...
                if (check_multiple !== Math.round(check_multiple)) {
                  ...
                  hardcoded_errors.push(
                    `The provided value for ${type} is not a multiple of ${process.env.store[type].per}. (${amount})`
                  );
                } else {
        ...
        if (!how_many_are_enabled)
          return functions.doRedirect(req, res, redirects.storeisdisabled);
        if (test_if_all_zero === how_many_are_enabled)
          return functions.doRedirect(
            req,
            res,
            redirects.allvaluescannotbezero
          );
