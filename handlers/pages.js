...
module.exports.load = async function (app, ifValidAPI, ejs) {
  ...
  app.get("*", async (req, res) => {
    ...
    if (req._parsedUrl.pathname === "/") {
      ...
      if (checkexist.length === 0) {
        ...
        if (permission === 0 || permission === 1 || permission === 2) {
          // Checks if it is a valid permission number.
          /*
          ...
          if (permission === 1 || permission === 2) {
            ...
            if (!req.session.data || !req.session.data.dbinfo) {
              if (pathexists.no_permission_redirect)
                return functions.doRedirect(
                  req,
                  res,
                  pathexists.no_permission_redirect
                );
              res.status(403);
              ...
              if (permission === 1) {
                ...
                if (!req.session.data.panelinfo.root_admin) {
                  // If user isn't an administrator.
                  // eslint-disable-next-line no-undef
                  if (pathexists.no_permission_redirect)
                    returnfunctions.doRedirect(
                      req,
                      res,
                      pathexists.no_permission_redirect
                    );
            ...
            // Pages anyone can go to. (no login required)
            file = pathexists.file; // The value of the variable "file" on the path name object is the page to be shown.
          }
        } else {
          // Permissions value on pages.yml for the path is not 0, 1, or 2.
          console.log(
    ...
    const variables = {
      // Creates "variables" object for what variables are to be sent frontend.
    ...
    ejs.renderFile(
      ...
      async function (err, str) {
        // Function ran after the file has successfully rendered.
        if (err) {
          ...
          ejs.renderFile(
            // This renders the rendering error EJS file.
            ...
            async function (err, str) {
              ...
              if (err) {
                ...
                console.log(
                  `[WEBSITE] An error has also occured while attempting to send the rendering error page on path ${req._parsedUrl.pathname}:`
                );
                console.log(err);
                return res.send(
                  "An error has occured while attempting to load this page. Please contact an administrator to fix this."
